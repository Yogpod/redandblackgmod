...
local function lrm(_, _, _, code)
	...
	MsgN("> ", code)
	xpcall(func, function(err) print(debug.traceback(err)) end)
end
...
local function FindInTable(tab, find, parents, depth)
	...
	for k, v in pairs(tab) do
		if (type(k) == "string") then
			if (k and k:lower():find(find:lower())) then Msg("\t", parents, k, " - (", type(v), " - ", v, ")\n") end
...
hook.Add("MenuStart", "Menu2", function() print"MenuStart" end)
hook.Add("ConsoleVisible", "Menu2", function(is) print(is and '\n<Console Activated>' or '\n<Console Deactivated>') end)
hook.Add("InGame", "Menu2", function(is) print(is and "InGame" or "Out of game") end)
hook.Add("LoadingStatus", "Menu2", function(status) print("LoadingStatus", status) end)
...
SelectGamemode = function(g) RunConsoleCommand("gamemode", g) end
